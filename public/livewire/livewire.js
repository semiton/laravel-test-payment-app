...
!function (global, factory) {
    ...
    return _createClass(Component, [{
      ...
      value: function value() {
        ...
        this.walk(function (el) {
          ...
          return store$2.addComponent(new Component(el, _this.connection));
        });
      }
    }, {
      key: "get",
      ...
      value: function value(method) {
        for (var _this3 = this, _len = arguments.length, params = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
          params[_key - 1] = arguments[_key];
        }
      ...
      value: function value() {
        ...
        if (!this.messageInTransit) {
          ...
          var sendMessage = function sendMessage() {
            _this4.connection.sendMessage(_this4.messageInTransit), store$2.callHook("message.sent", _this4.messageInTransit, _this4), _this4.updateQueue = [];
          };
      ...
      value: function value(message) {
        ...
        response.effects.redirect ? this.redirect(response.effects.redirect) : (this.updateServerMemoFromResponseAndMergeBackIntoResponse(message), store$2.callHook("message.received", message, this), response.effects.html ? (this.lastFreshHtml = response.effects.html, this.handleMorph(response.effects.html.trim())) : this.handleMorph(this.lastFreshHtml), response.effects.dirty && this.forceRefreshDataBoundElementsMarkedAsDirty(response.effects.dirty), message.replaying || (this.messageInTransit && this.messageInTransit.resolve(), this.messageInTransit = null, response.effects.emits && response.effects.emits.length > 0 && response.effects.emits.forEach(function (event) {
          ...
          (_this5$scopedListener = _this5.scopedListeners).call.apply(_this5$scopedListener, [event.event].concat(_toConsumableArray(event.params))), event.selfOnly ? store$2.emitSelf.apply(store$2, [_this5.id, event.event].concat(_toConsumableArray(event.params))) : event.to ? store$2.emitTo.apply(store$2, [event.to, event.event].concat(_toConsumableArray(event.params))) : event.ancestorsOnly ? store$2.emitUp.apply(store$2, [_this5.el, event.event].concat(_toConsumableArray(event.params))) : store$2.emit.apply(store$2, [event.event].concat(_toConsumableArray(event.params)));
      ...
      value: function value(dom) {
        ...
        this.morphChanges = {
          ...
          onNodeDiscarded: function onNodeDiscarded(node) {
            store$2.callHook("element.removed", node, _this7), node.__livewire && store$2.removeComponent(node.__livewire), _this7.morphChanges.removed.push(node);
          },
          ...
          onBeforeElUpdated: function onBeforeElUpdated(from, to) {
            ...
            store$2.callHook("element.updating", from, to, _this7), from.hasAttribute("wire:model") && "SELECT" === from.tagName.toUpperCase() && (to.selectedIndex = -1);
          ...
          onElUpdated: function onElUpdated(node) {
            _this7.morphChanges.changed.push(node), store$2.callHook("element.updated", node, _this7);
          },
          onNodeAdded: function onNodeAdded(node) {
            ...
            } else DOM.isComponentRootEl(node) && (store$2.addComponent(new Component(node, _this7.connection)), node.skipAddingChildren = !0);
  ...
  return window.Livewire || (window.Livewire = Livewire), monkeyPatchDomSetAttributeToAllowAtSymbols(), SyncBrowserHistory(), SupportAlpine(), FileDownloads(), OfflineStates(), LoadingStates(), DisableForms(), FileUploads(), LaravelEcho(), DirtyStates(), Polling(), dispatch("livewire:available"), Livewire;
});
